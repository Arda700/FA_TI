...
with serial.Serial(port=pico_port, baudrate=115200, bytesize=8, parity='N', stopbits=1, timeout=1) as serial_port:
    ...
    try:
        # Request user input
        commands = ['off', 'on', 'temperature', 'exit']
        while True:
            ...
            if choice == 'off':
                ...
                data = "2\r"  # You can choose any unique identifier for the temperature request, here using '2'
                serial_port.write(data.encode())
                pico_output = read_serial(serial_port)
                pico_output = pico_output.replace('\r\n', ' ')
                print("[PICO] Temperature: " + pico_output + "degrees Celsius")
            elif choice == 'exit':
